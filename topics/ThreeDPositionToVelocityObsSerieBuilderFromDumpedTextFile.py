...
from mDynamicSystem.measurement.ThreeDPosVelObsSerieBuilder import ThreeDPosVelObsSerieBuilder


class ThreeDPositionToVelocityObsSerieBuilderFromDumpedTextFile:
    '''This class is responsible for Building and measurement serie from a topic dumped text file and save it
    '''
        ...
        if (self.__obsSerie == None):
            ...
            for lineCounter, curLine in enumerate(fileToReadFrom):
                if lineCounter > self.__linelimit:
                    ...
                    continue
                prvObs = threeDPosVelObsSerieBuilder.getObservationSerie().getByMeasurementByIndex(lineCounter - 2)
                ...
                if lineCounter == 2:
                    firstThreeDPosVelObs = threeDPosVelObsSerieBuilder.getObservationSerie().getByMeasurementByIndex(0)
    ...
    def saveToFileWithTime(self, filePath: str, sep=","):
        ...
        with open(filePath, 'w') as f:
            for obs in self.getObsSerie().getActionList():
